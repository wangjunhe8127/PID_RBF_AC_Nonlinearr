...
class Distribution(ABC):
    ...
    def get_actions(self, deterministic: bool = False,is_rsample:bool=False) -> th.Tensor:
        """
        ...
        if deterministic:
            return self.mode()
#         print(self.sample(is_rsample))
        return self.sample(is_rsample)
...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution_net(self, latent_dim: int, log_std_init: float = 0.0, action_num:int = 1) -> Tuple[nn.Module, nn.Parameter]:
        ...
        :return:
        """
        mean_actions = nn.Linear(latent_dim, self.action_dim).to(th.device("cuda"))

#         mean_actions = nn.Linear(latent_dim, self.action_dim)
        # TODO: allow action dependent std
        log_std = nn.Parameter(th.ones(self.action_dim) * log_std_init, requires_grad=True)
        return mean_actions, log_std
    def proba_distribution(self, mean_actions: th.Tensor, log_std: th.Tensor,value:th.Tensor,need_grad:bool,need_noise:bool) -> "DiagGaussianDistribution":
        """
        ...
        """
        # 扰动的方差
        theta = 1/(1+(2*value).exp())
#         print(value,theta)
        # 扰动采样
        tmp = th.normal(0,theta) if need_grad else th.normal(0,theta).detach()
        # 由于v太小，导致扰动大，所以加系数
        tmp = 0.001*tmp
        # 每一维度都加上扰动
        if need_noise:
            new_actions = mean_actions+tmp
        else:
            new_actions = mean_actions

#         print('mean',mean_actions,'new',new_actions)
#         print('fdfdfdfdsfsdfsd',tmp,mean_actions,new_actions)
        action_std = th.ones_like(new_actions) * log_std.exp()
#         print(action_std,new_actions)
        self.distribution = Normal(new_actions, action_std)
#         print()
        return self
    ...
    def sample(self,is_rsample) -> th.Tensor:
