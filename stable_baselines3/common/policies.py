...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        self.value_net = nn.Linear(self.mlp_extractor.latent_dim_vf, 1).to(th.device("cuda"))
...
#         print(latent_pi.device, latent_vf)
        # Evaluate the values for the given observations
#         print(latent_vf.grad)
#         print(self.value_net.parameters())
#         for _,param in enumerate(self.value_net.named_parameters()):
#             print(param[0])
#             print(param[1])
#             print('----------------')
        values = self.value_net(latent_vf)
#         print(values.grad)
        distribution = self._get_action_dist_from_latent(latent_pi,values,self.need_grad,self.need_noise)
        actions = distribution.get_actions(deterministic=deterministic,is_rsample=self.is_rsample)
        
#         print(actions,deterministic)
        log_prob = distribution.log_prob(actions)
        return actions, values, log_prob

    def _get_action_dist_from_latent(self, latent_pi: th.Tensor,value:th.Tensor,need_grad:bool,need_noise:bool) -> Distribution:
        """
...
#         print(mean_actions.grad)
        if isinstance(self.action_dist, DiagGaussianDistribution):
            return self.action_dist.proba_distribution(mean_actions, self.log_std, value,need_grad,need_noise)
    ...
    def get_distribution(self, obs: th.Tensor) -> Distribution:
        ...
        latent_pi, latent_vf = self.mlp_extractor(features)
        values = self.value_net(latent_vf)
        return self._get_action_dist_from_latent(latent_pi,values,self.need_grad,self.need_noise)
